...
async def rejoiner(event):
    ...
    Songs = Player(chat)
    try:
        await Songs.group_call.reconnect()
...
async def volume_setter(event):
    ...
    if vol:
        Songs = Player(chat)
        await Songs.group_call.set_my_volume(int(vol))
        if vol > 200:
            vol = 200
        elif vol < 1:
            vol = 1
