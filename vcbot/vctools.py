...
async def mute(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    Songs = Player(chat)
    await Songs.group_call.set_is_mute(True)
...
async def unmute(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    Songs = Player(chat)
    await Songs.group_call.set_is_mute(False)
...
async def pauser(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    Songs = Player(chat)
    await Songs.group_call.set_pause(True)
...
async def resumer(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    Songs = Player(chat)
    await Songs.group_call.set_pause(False)
...
async def replayer(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    Songs = Player(chat)
    Songs.group_call.restart_playout()
