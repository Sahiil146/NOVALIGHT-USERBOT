...
async def play_music_(event):
    ...
    Songs = Player(chat, event)
    song_name = f"{song_name[:30]}..."
    if not Songs.group_call.is_connected:
        if not (await Songs.vc_joiner()):
            return
        await Songs.group_call.start_audio(song)
...
async def radio_mirchi(e):
    ...
    Songs = Player(chat, e)
    if not Songs.group_call.is_connected and not (await Songs.vc_joiner()):
        return
    await Songs.group_call.start_audio(song)
    await xx.reply(
        f"• Started Radio 📻\n\n• Station : `{song}`",
...
async def live_stream(e):
    ...
    await xx.delete()
    await Songs.group_call.start_audio(file)
