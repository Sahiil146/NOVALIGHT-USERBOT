...
from LightUB._misc import SUDO_M, owner_and_sudos
from LightUB.fns.helper import inline_mention
from strings import get_string
from LightUB.version import __version__
from LightUB.dB.asst_fns import *
from LightUB.dB import LIGHT_IMAGES
...
def __getmedia():
    media = udB.get_key("STARTMEDIA")
    if media == False:
        return None
    elif not media:
        return choice(LIGHT_IMAGES)
    return media


@asst_cmd(pattern="start( (.*)|$)", forwards=False, func=lambda x: not x.is_group)
async def _light(event):
    ...
    if event.sender_id not in SUDO_M.fullsudos:
        ok = ""
        me = inline_mention(light_bot.me)
        ...
        if not udB.get_key("STARTMSG"):
            ...
            await event.reply(
                f"Hey there {mention}, this is Assistant Bot of {me}!\n\n{ok}",
                file=__getmedia(),
