...
try:
    from LightUB.fns.gDrive import GDriveManager
except ImportError:
    GDriveManager = None
...
from LightUB.fns.helper import fast_download, progress
from LightUB.fns.tools import Carbon, async_searcher, get_paste, telegraph_client
from LightUB.startup.loader import Loader
...
async def update(eve):
    ...
    if heroku_api:
        ...
        execl(sys.executable, sys.executable, "-m", "LightUB")
    ...
    if not match:
        try:
            ...
            return await okk.edit(
                f"**• light Userbot •**{cli}", file=img, buttons=button
            )
        except Exception as er:
            LOGS.exception(er)
...
async def _(e):
    ...
    async with asst.conversation(e.sender_id) as conv:
        ...
        if GDrive._create_token_file(code=code.text):
            await conv.send_message(
                "`Success!\nYou are all set to use Google Drive with light Userbot.`",
...
async def _(e):
    if not e.is_private:
        return
    await e.edit(
        "Click Authorise and send the code.\n\nYou can use your own CLIENT ID and SECRET by [this](https://t.me/CodeByTyagi/37)",
        buttons=[
            [
