...
from LightUB.dB.forcesub_db import add_forcesub, get_forcesetting, rem_forcesub
...
@light_cmd(pattern="fsub( (.*)|$)", admins_only=True, groups_only=True)
async def addfor(e):
    ...
    light_bot.add_handler(force_sub, events.NewMessage(incoming=True))


@light_cmd(pattern="remfsub$")
async def remor(e):
...
async def fcall(e):
    match = e.pattern_match.group(1).strip()
    spli = match.split("_")
    user = await light_bot.get_entity(int(spli[0]))
    cl = await light_bot.get_entity(int(spli[1]))
    text = f"Hi {inline_mention(user)}, You Need to Join"
    text += f" {cl.title} in order to Chat in this Group."
    el = (
        f"https://t.me/{cl.username}"
        if cl.username
        else (await light_bot(ExportChatInviteRequest(cl))).link
    )
...
async def diesoon(e):
    ...
    try:
        values = await light_bot(GetParticipantRequest(int(spli[1]), int(spli[0])))
...
async def force_sub(ult):
    ...
    if count in range(2, 11):
        return
    try:
        await light_bot.get_permissions(int(joinchat), user.id)
        return
    except UserNotParticipantError:
        pass
    if isinstance(user, Channel):
        try:
            await light_bot.edit_permissions(ult.chat_id, user.id, view_messages=False)
            return
        except BaseException as er:
            LOGS.exception(er)
    try:
        await light_bot.edit_permissions(ult.chat_id, user.id, send_messages=False)
    except ChatAdminRequiredError:
        return
    except Exception as e:
        await ult.delete()
        LOGS.info(e)
    res = await light_bot.inline_query(asst.me.username, f"fsub {user.id}_{joinchat}")
