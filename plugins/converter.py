...
async def _(e):
    ...
    await bash(f"wget {nn} -O resources/extras/light.jpg")
...
async def uconverter(event):
    ...
    file = await con.convert(b, outname="light", convert_to=convert)
    if file:
...
async def _(event):
    ...
    await event.reply(file=input_str, thumb=LightConfig.thumb)
    await xx.delete()
    os.remove(input_str)
