...
from . import LOGS, get_help, get_string, udB, light_bot, light_cmd

__doc__ = get_help("help_autopic")


@light_cmd(pattern="autopic( (.*)|$)")
async def autopic(e):
...
if search:
    ...
    async def autopic_func():
        ...
        file = await light_bot.upload_file(img)
        await light_bot(UploadProfilePhotoRequest(file))
        shuffle(ok)
