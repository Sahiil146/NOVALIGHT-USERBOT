...
from . import LOGS, asst, eor, events, get_string, udB, light_bot, light_cmd
...
async def source(e):
    ...
    if not is_source_channel_added(y):
        ...
        light_bot.add_handler(autopost_func, events.NewMessage())
    elif is_source_channel_added(y):
        await e.eor(get_string("cha_3"))


@light_cmd(pattern="dsource( (.*)|$)")
async def dd(event):
...
@light_cmd(pattern="listdest")
async def list_all(event):
    light_bot = event.client
    x = await event.eor(get_string("com_1"))
    ...
    for channel in channels:
        name = ""
        try:
            name = get_display_name(await light_bot.get_entity(int(channel)))
        except BaseException:
            name = ""
        msg += f"\n=> **{name}** [`{channel}`]"
