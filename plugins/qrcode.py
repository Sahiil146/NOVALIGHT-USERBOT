...
from LightUB import LightConfig
...
from . import check_filename, get_string, light_bot, light_cmd


@light_cmd(pattern="qrcode( (.*)|$)")
async def cd(e):
    ...
    default, cimg = LightConfig.thumb, None
    if reply and (reply.sticker or reply.photo):
        cimg = await reply.download_media()
    elif light_bot.me.photo and not light_bot.me.photo.has_video:
        cimg = await e.client.get_profile_photos(light_bot.uid, limit=1)[0]
