...
from LightUB.dB.gban_mute_db import (
    gban,
    gmute,
    is_gbanned,
    is_gmuted,
...
from LightUB.dB.gcast_blacklist_db import (
    add_gblacklist,
    is_gblacklisted,
    rem_gblacklist,
)
...
@light_cmd(pattern="gpromote( (.*)|$)", fullsudo=True)
async def _(e):
...
@light_cmd(pattern="gdemote( (.*)|$)", fullsudo=True)
async def _(e):
    x = e.pattern_match.group(1).strip()
    light_bot = e.client
    if not x:
    ...
    if user:
        ...
        if user.isdigit():
            user = int(user)
        try:
            name = await light_bot.get_entity(user)
        except BaseException:
            return await e.eor(f"`No User Found Regarding {user}`", time=5)
        ...
        rank = "Not AdMin"
        c = 0
        async for x in light_bot.iter_dialogs():
            if (
                "group" in key.lower()
                and x.is_group
                or "group" not in key.lower()
...
@light_cmd(pattern="ungban( (.*)|$)", fullsudo=True)
async def _(e):
...
@light_cmd(pattern="gban( (.*)|$)", fullsudo=True)
async def _(e):
...
@light_cmd(pattern="g(admin|)cast( (.*)|$)", fullsudo=True)
...
@light_cmd(pattern="gucast( (.*)|$)", fullsudo=True)
...
@light_cmd(pattern="gkick( (.*)|$)", fullsudo=True)
async def gkick(e):
...
@light_cmd(pattern="gmute( (.*)|$)", fullsudo=True)
async def _(e):
...
@light_cmd(pattern="ungmute( (.*)|$)", fullsudo=True)
async def _(e):
...
async def list_gengbanned(event):
    ...
    if len(gbanned_users) > 4096:
        ...
        await x.reply(
            file="gbanned.txt",
            message=f"List of users GBanned by {inline_mention(light_bot.me)}",
        )
...
@light_cmd(pattern="gblacklist$")
async def blacklist_(event):
    await gblacker(event, "add")


@light_cmd(pattern="ungblacklist$")
