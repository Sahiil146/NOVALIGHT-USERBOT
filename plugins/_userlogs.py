...
async def all_messages_catcher(e):
    ...
    try:
        ...
        if TAG_EDITS.get(e.chat_id):
            TAG_EDITS[e.chat_id].update({e.id: {"id": sent.id, "msg": e.text}})
        else:
            TAG_EDITS.update({e.chat_id: {e.id: {"id": sent.id, "msg": e.text}}})
                ...
                try:
                    ...
                    if TAG_EDITS.get(e.chat_id):
                        TAG_EDITS[e.chat_id].update(
                            {e.id: {"id": sent.id, "msg": e.text}}
                        )
                    else:
                        TAG_EDITS.update(
                            {e.chat_id: {e.id: {"id": sent.id, "msg": e.text}}}
                        )
...
if udB.get_key("TAG_LOG"):
    ...
    @light_bot.on(events.MessageEdited(func=lambda x: not x.out))
...
_client = {"bot": asst, "user": light_bot}
