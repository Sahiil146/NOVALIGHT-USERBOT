...
from LightUB.dB import autoban_db, dnd_db
from LightUB.fns.admins import get_update_linked_chat

from . import LOGS, asst, get_string, inline_mention, light_bot, light_cmd
...
@light_cmd(pattern="cban$", admins_only=True)
...
@light_cmd(pattern="(list|add|rem)wl( (.*)|$)")
...
if dnd_db.get_dnd_chats():
    light_bot.add_handler(dnd_func, events.ChatAction(func=lambda x: x.user_joined))
