...
from LightUB._misc._assistant import callback, in_pattern
from LightUB.dB._core import HELP, LIST
from LightUB.fns.helper import gen_chlog, time_formatter, updater
from LightUB.fns.misc import split_list
...
SUP_BUTTONS = [
    [
        Button.url("• Repo •", url="https://github.com/CodeWithTyagi/light"),
        Button.url("• Support •", url="t.me/CodeWithTyagi"),
    ],
]
...
async def inline_alive(o):
    TLINK = inline_pic() or "https://graph.org/file/6fd3a5a6a7dd77684d01d.jpg"
    MSG = "• **Light UserBot •**"
    WEB0 = InputWebDocument(
        "https://graph.org/file/26235bffb5d8ee50f882c.jpg", 0, "image/jpg", []
    )
    RES = [
...
async def inline_handler(event):
    ...
    if inline_pic():
        ...
        result = await event.builder.article(
            title="Light Help Menu", text=text, buttons=_main_help_menu
        )
...
async def _(event):
    ...
    if len(changelog_str) > 1024:
        ...
        with open("light_updates.txt", "w+") as file:
            file.write(tl_chnglog)
        await event.edit(
            get_string("upd_5"),
            file="light_updates.txt",
            buttons=[
                [Button.inline("• Uᴘᴅᴀᴛᴇ Nᴏᴡ •", data="updatenow")],
                [Button.inline("« Bᴀᴄᴋ", data="ownr")],
            ],
        )
        remove("light_updates.txt")
    else:
        await event.edit(
            changelog_str,
            buttons=[
...
async def ibuild(e):
    ...
    if pic:
        try:
            ...
            LOGS.exception(er)
    result = [
        await builder.article("Light OP", text=txt, link_preview=False, buttons=btn)
    ]
