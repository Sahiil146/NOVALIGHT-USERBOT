...
from LightUB.dB import DEVLIST
from LightUB.dB.logusers_db import *
from LightUB.dB.pmpermit_db import *
...
if udB.get_key("PMSETTING"):
    if udB.get_key("AUTOAPPROVE"):
        ...
        async def autoappr(e):
            ...
            try:
                await light_bot.edit_folder(miss.id, folder=0)
    ...
    async def permitpm(event):
        ...
        if not is_approved(user.id) and event.text != UND:
            if Redis("MOVE_ARCHIVE"):
                try:
                    await light_bot.edit_folder(user.id, folder=1)
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    ...
                    if inline_pm:
                        results = await light_bot.inline_query(my_bot, f"ip_{user.id}")
                        try:
                        ...
                        _to_delete[user.id] = await light_bot.send_file(
                            user.id,
                            PMPIC,
                            caption=message_,
                        )
                    else:
                        _to_delete[user.id] = await light_bot.send_message(
                            user.id, message_
                        )
                    ...
                    if inline_pm:
                        try:
                            results = await light_bot.inline_query(
            ...
            if COUNT_PM[user.id] >= WARNS:
                ...
                await light_bot(BlockRequest(user.id))
                await light_bot(ReportSpamRequest(peer=user.id))
                await asst.edit_message(
                    int(udB.get_key("LOG_CHANNEL")),
                    _not_approved[user.id],
    ...
    @light_cmd(pattern="(start|stop|clear)archive$", fullsudo=True)
    async def _(e):
    ...
    @light_cmd(pattern="(a|approve)(?: |$)", fullsudo=True)
    ...
    @light_cmd(pattern="(da|disapprove)(?: |$)", fullsudo=True)
...
@light_cmd(pattern="block( (.*)|$)", fullsudo=True)
...
@light_cmd(pattern="unblock( (.*)|$)", fullsudo=True)
...
@light_cmd(pattern="listapproved$", owner=True)
async def list_approved(event):
    ...
    for i in all:
        try:
            name = get_display_name(await light_bot.get_entity(i))
        except BaseException:
            name = ""
...
async def apr_in(event):
    ...
    if not is_approved(uid):
        approve_user(uid)
        try:
            await light_bot.edit_folder(uid, folder=0)
        except BaseException:
            pass
        try:
            user = await light_bot.get_entity(uid)
        except BaseException:
...
async def blck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await light_bot(BlockRequest(uid))
    except BaseException:
        pass
    try:
        user = await light_bot.get_entity(uid)
...
async def unblck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await light_bot(UnblockRequest(uid))
    except BaseException:
        pass
    try:
        user = await light_bot.get_entity(uid)
...
async def ytfuxist(e):
    try:
        ...
        await light_bot.delete_messages(e.chat_id, e.id)
...
async def in_pm_ans(event):
    ...
    await event.answer(res, switch_pm="• light •", switch_pm_param="start")


@callback(re.compile("admin_only(.*)"), from_users=[light_bot.uid])
