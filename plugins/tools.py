...
from LightUB.fns.tools import metadata, translate
...
from . import inline_mention, is_url_ok, mediainfo, light_cmd


@light_cmd(pattern="tr( (.*)|$)", manager=True)
async def _(event):
...
@light_cmd(pattern="bots( (.*)|$)", groups_only=True, manager=True)
async def _(ult):
...
async def _(e):
    ...
    try:
        ...
        with io.BytesIO(str.encode(text)) as out_file:
            ...
            await e.reply(f"`{e.text}`", file=out_file, thumb=LightConfig.thumb)
        await e.delete()
