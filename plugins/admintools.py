...
@light_cmd(pattern="pin$", manager=True, require="pin_messages", fullsudo=True)
async def pin(msg):
    if not msg.is_reply:
...
@light_cmd(pattern="purge( (.*)|$)", manager=True, require="delete_messages")
...
@light_cmd(pattern="pinned", manager=True, groups_only=True)
