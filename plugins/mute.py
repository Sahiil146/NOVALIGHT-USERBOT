...
from LightUB.dB.mute_db import is_muted, mute, unmute
from LightUB.fns.admins import ban_time

from . import asst, eod, get_string, inline_mention, light_bot, light_cmd
...
async def startmute(event):
    ...
    if input_:
        ...
        if reply.out or userid in [light_bot.me.id, asst.me.id]:
...
@light_cmd(pattern="mute( (.*)|$)", admins_only=True, manager=True, require="ban_users")
async def _(e):
    xx = await e.eor("`Muting...`")
    input = e.pattern_match.group(1).strip()
    chat = await e.get_chat()
