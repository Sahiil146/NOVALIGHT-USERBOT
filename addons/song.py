...
@light_cmd(pattern="song ?(.*)")
async def _(event):
    light_bot = event.client
    try:
        await light_bot(ImportChatInviteRequest("DdR2SUvJPBouSW4QlbJU4g"))
    ...
    try:
        async for event in light_bot.iter_messages(
            chat, search=args, limit=1, filter=filtermus
        ):
            await light_bot.send_file(current_chat, event, caption=event.message)
        await okla.delete()
    except Exception:
