...
@light_cmd(pattern="clone ?(.*)", fullsudo=True)
async def _(event):
    eve = await event.eor("`Processing...`")
    reply_message = await event.get_reply_message()
    whoiam = await event.client(GetFullUserRequest(light_bot.uid))
    if whoiam.full_user.about:
        mybio = str(light_bot.me.id) + "01"
        udB.set_key(f"{mybio}", whoiam.full_user.about)  # saving bio for revert
    udB.set_key(f"{light_bot.uid}02", whoiam.users[0].first_name)
    if whoiam.users[0].last_name:
        udB.set_key(f"{light_bot.uid}03", whoiam.users[0].last_name)
...
async def _(event):
    ...
    if chc:
        bio = chc
    fname = udB.get_key(f"{light_bot.uid}02")
    lname = udB.get_key(f"{light_bot.uid}03")
    if fname:
        name = fname
    if lname:
        ok = lname
    ...
    udB.del_key(f"{light_bot.uid}01")
    udB.del_key(f"{light_bot.uid}02")
    udB.del_key(f"{light_bot.uid}03")
