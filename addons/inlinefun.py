...
• `{i}t2g <text>`
    create fancy gif with text.
"""

from random import choice

from . import light_cmd, get_string, deEmojify


@light_cmd(pattern="tweet ?(.*)")
async def tweet(e):
    wai = await e.eor("...")
...
async def honkasays(e):
    ...
    try:
        ...
        await e.reply("Honka", file=choice(res).document)
        await wai.delete()
    except Exception as er:
        await wai.edit(str(er))


@light_cmd(pattern="uta ?(.*)")
async def nope(doit):
...
@light_cmd(pattern="t2g ?(.*)")
async def text2gif(e):
    wai = await e.eor("...")
    text = e.pattern_match.group(1)
    if not text:
        return await wai.edit("`Give me Some Text !`")
    text = deEmojify(text)
    try:
        res = await e.client.inline_query("text2gifBot", text)
        await e.reply("Light UB", file=choice(res).document)
        await wai.delete()
    except Exception as m:
        await e.eor(str(m))
