...
async def get_chatbot_reply(message):
    ...
    chatbot_base = "https://kukiapi.xyz/api/apikey=lightUSERBOT/light/{}/message={}"
    req_link = chatbot_base.format(
        light_bot.me.first_name or "light user",
        message,
    )
    try:
...
def telegraph_client():
    ...
    gd_name = light_bot.full_name
    short_name = gd_name[:30]
    profile_url = (
        f"https://t.me/{light_bot.me.username}"
        if light_bot.me.username
        else "https://t.me/CodeByTyagi"
    )
    try:
        ...
        if "SHORT_NAME_TOO_LONG" in str(er):
            TelegraphClient.create_account(
                short_name="lightuser", author_name=gd_name, author_url=profile_url
            )
        else:
            LOGS.exception(er)
            return
...
async def Carbon(
    ...
    **kwargs,
    ...
    if rayso:
        ...
        kwargs["title"] = kwargs.get("title", "light")
    else:
        kwargs["code"] = code
