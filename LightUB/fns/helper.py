...
async def safeinstall(event):
    ...
    if plug in HELP:
        ...
        for i in HELP[plug]:
            output += i
        output += "\n© @CodeByTyagi"
        await eod(ok, f"✓ `light - Installed`: `{plug}` ✓\n\n{output}")
...
async def heroku_logs(event):
    ...
    ok = app.get_log()
    with open("light-heroku.log", "w") as log:
        log.write(ok)
    await event.client.send_file(
        event.chat_id,
        file="light-heroku.log",
        thumb=LightConfig.thumb,
        caption="**light Heroku Logs.**",
    )

    os.remove("light-heroku.log")
    await xx.delete()


async def def_logs(ult, file):
    from .. import LightConfig
...
def gen_chlog(repo, diff):
    ...
    ch_log = tldr_log = ""
    ch = f"<b>Light {__version__} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
    ch_tl = f"Light {__version__} updates for {ac_br}:"
    d_form = "%d/%m/%y || %H:%M"
...
async def progress(current, total, event, start, type_of_ps, file_name=None):
    jost = str(event.chat_id) + str(event.id)
