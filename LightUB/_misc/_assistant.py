...
from .. import LOGS, asst, udB, light_bot
from ..fns.admins import admin_check
from . import append_or_update, owner_and_sudos


OWNER = light_bot.full_name

MSG = f"""
**Light - UserBot**
➖➖➖➖➖➖➖➖➖➖
**Owner**: [{OWNER}](tg://user?id={light_bot.uid})
**Support**: @CodeByTyagi
➖➖➖➖➖➖➖➖➖➖
"""
...
IN_BTTS = [
    [
        Button.url(
            "Repository",
            url="https://github.com/CodeWithTyagi/light",
        ),
        Button.url("Support", url="https://t.me/CodeWithTyagi"),
    ]
]
...
def callback(data=None, from_users=[], admins=False, owner=False, **kwargs):
    ...
    if "me" in from_users:
        ...
        from_users.append(light_bot.uid)
            ...
            if owner and event.sender_id not in owner_and_sudos():
                res = [
                    await event.builder.article(
                        ...
                        description="(c) CodeByTyagi",
                        text=MSG,
                        thumb=InputWebDocument(
                            "https://graph.org/file/10528616745918b827cf3.jpg",
                            0,
                            "image/jpeg",
                            [],
                        ),
                    ...
                    await event.answer(
                        [
                            await event.builder.article(
                                ...
                                text=error_text(),
                                buttons=Button.url(
                                    "Report", "https://t.me/CodeWithTyagi"
                                ),
                            )
                        ]
                    )
                except (QueryIdInvalidError, ResultIdDuplicateError) as exc:
                    LOGS.exception(exc)
                except Exception as er:
                    LOGS.exception(er)
