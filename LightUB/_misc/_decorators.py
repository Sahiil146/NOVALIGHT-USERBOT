...
def light_cmd(pattern=None, manager=False, light_bot=light_bot, asst=asst, **kwargs):
    ...
    def decor(dec):
        async def wrapp(event):
            if not event.out:
                ...
                if fullsudo and event.sender_id not in SUDO_M.fullsudos:
                    return await eod(event, get_string("py_d2"), time=15)
            chat = event.chat
            if admins_only:
                if event.is_private:
                    return await eod(event, get_string("py_d3"))
                if not (chat.admin_rights or chat.creator):
                    return await eod(event, get_string("py_d5"))
            ...
            try:
                ...
                await light_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await light_bot.connect()
                await asst.send_message(
                    udB.get_key("LOG_CHANNEL"),
                ...
                return await eod(event, get_string("py_d8"))
            except (BotMethodInvalidError, UserIsBotError):
                return await eod(event, get_string("py_d6"))
                ...
                await asst.send_message(
                    ...
                    buttons=[
                        ...
                        Button.url(
                            "Repl",
                            "https://replit.com/@Thelight/lightStringSession",
                        ),
                    ],
                )
                sys.exit()
                ...
                ftext = "**LightBot Error:** `Forward this to` @CodeWithTyagi\n\n"
                ftext += "\n**Light Bot Version:** " + str(__version__)
                ftext += "\n**Telethon Version:** " + str(telever)
                ftext += f"\n**Hosted At:** `{HOSTED_ON}`\n\n"
                ftext += "------ LightBot CRASH LOG ------"
                ftext += "\n**Date:** " + date
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**LightBot Error:** `Forward this to` @CodeWithTyagi\n\n",
                        )
                else:
        ...
        if manager and MANAGER:
            ...
            async def manager_cmd(event):
                if not allow_all and not (await admin_check(event, require=require)):
                    return
                ...
                try:
                    ...
                    chat = udB.get_key("MANAGER_LOG")
                    if chat:
                        text = f"**#MANAGER_LOG\n\nChat:** `{get_display_name(event.chat)}` `{event.chat_id}`"
                        text += f"\n**Replied :** `{event.is_reply}`\n**Command :** {event.text}\n\n**Error :** `{er}`"
                        try:
                            ...
                            LOGS.exception(er)
                    LOGS.info(f"• MANAGER [{event.chat_id}]:")
                    LOGS.exception(er)
